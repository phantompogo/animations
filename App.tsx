...
const DEFAULT_API_KEY = process.env.API_KEY; // This will be undefined if not set at build time
const LOCAL_STORAGE_API_KEY_OPTION = 'GEMINI_API_KEY_OPTION';
const LOCAL_STORAGE_USER_API_KEY = 'GEMINI_USER_API_KEY';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const savedOption = localStorage.getItem(LOCAL_STORAGE_API_KEY_OPTION) as 'default' | 'custom' | null;
    const savedUserKey = localStorage.getItem(LOCAL_STORAGE_USER_API_KEY);

    if (savedOption) {
      setApiKeyOption(savedOption);
    }
    if (savedUserKey) {
      setStoredUserApiKey(savedUserKey);
      if (savedOption === 'custom') {
        setUserApiKeyInput(savedUserKey);
      }
    }
  }, []);

  const handleApiOptionChange = useCallback((option: 'default' | 'custom') => {
    setApiKeyOption(option);
    localStorage.setItem(LOCAL_STORAGE_API_KEY_OPTION, option);
    if (option === 'custom' && storedUserApiKey) {
      setUserApiKeyInput(storedUserApiKey);
    } else if (option === 'default') {
      // Optionally clear input when switching to default, or leave it
      // setUserApiKeyInput(''); 
    }
    setAlert(null);
  }, [storedUserApiKey]);

  const handleUserApiKeyInputChange = useCallback((key: string) => {
    setUserApiKeyInput(key);
  }, []);

  const handleSaveUserApiKey = useCallback(() => {
    if (!userApiKeyInput.trim()) {
      setAlert({ message: 'API Key tidak boleh kosong.', type: AlertType.Error });
      return;
    }
    setStoredUserApiKey(userApiKeyInput);
    localStorage.setItem(LOCAL_STORAGE_USER_API_KEY, userApiKeyInput);
    setAlert({ message: 'API Key berhasil disimpan di browser Anda.', type: AlertType.Success });
  }, [userApiKeyInput]);

  const handleDeleteUserApiKey = useCallback(() => {
    localStorage.removeItem(LOCAL_STORAGE_USER_API_KEY);
    setStoredUserApiKey(null);
    setUserApiKeyInput('');
    setAlert({ message: 'API Key berhasil dihapus dari browser Anda.', type: AlertType.Info });
  }, []);
  ...
  const handleAnalyzeImage = useCallback(async () => {
    ...
    if (apiKeyOption === 'default') {
      apiKeyToUse = DEFAULT_API_KEY;
      if (!apiKeyToUse || apiKeyToUse === "MISSING_API_KEY") {
        setAlert({ message: 'API Key Utama tidak dikonfigurasi. Silakan pilih "Gunakan API Saya Sendiri" atau konfigurasikan API Utama.', type: AlertType.Error });
        setIsLoading(false);
        return;
      }
    } else { // custom
      apiKeyToUse = userApiKeyInput.trim() || storedUserApiKey;
      if (!apiKeyToUse) {
        setAlert({ message: 'Silakan masukkan API Key Anda atau pastikan API Key sudah tersimpan.', type: AlertType.Error });
        setIsLoading(false);
        return;
      }
    }
    ...
    try {
      const promptText = await analyzeImageWithGemini(
        apiKeyToUse, // Pass the selected API key
        imageBase64, 
      ...
      setAlert({ message: `Error: ${errorMessage}.`, type: AlertType.Error });
    } finally {
      setIsLoading(false);
    }
  }, [selectedImageFile, imageBase64, detailLevel, isModel3D, isModelAsli, isModelAnime, isModelGreenScreen, apiKeyOption, userApiKeyInput, storedUserApiKey]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-indigo-900 text-gray-100 flex flex-col items-center justify-center p-4 sm:p-6 font-sans selection:bg-sky-400 selection:text-sky-900">
        <div className="bg-slate-800/70 backdrop-blur-md shadow-2xl rounded-xl p-6 md:p-10 w-full max-w-2xl space-y-8 transform transition-all duration-500 hover:shadow-sky-500/20">
          ...
          <ApiKeyManager
            ...
            storedUserApiKey={storedUserApiKey}
            onApiOptionChange={handleApiOptionChange}
            onUserApiKeyInputChange={handleUserApiKeyInputChange}
            onSaveUserApiKey={handleSaveUserApiKey}
            onDeleteUserApiKey={handleDeleteUserApiKey}
            isLoading={isLoading}
            isDefaultKeyAvailable={!!DEFAULT_API_KEY && DEFAULT_API_KEY !== "MISSING_API_KEY"}
          />

          {alert && ( // Show general alerts above image uploader if they are not related to API key saving.
            (!alert.message.toLowerCase().includes("api key")) &&
            <AlertMessage
          ...
          {isLoading && <LoadingSpinner />}
           ...
           {alert && // Show API key related alerts (or general alerts if not filtered above)
             (alert.message.toLowerCase().includes("api key") || 
              (alert.message && !alert.message.toLowerCase().includes("generated successfully") && !alert.message.toLowerCase().includes("animation prompt generated"))) &&
            <AlertMessage
              message={alert.message}
              type={alert.type}
              onClose={clearAlert}
